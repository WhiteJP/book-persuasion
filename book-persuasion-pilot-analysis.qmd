...
## Structure of Lewis Outcome Measures (Pre-Treatment)

This section examines the structure of the IRS/federal worker attitude measures at baseline (pre-treatment) using a correlation matrix and Principal Component Analysis (PCA). This helps us understand how tightly these measures hang together and whether they form distinct factors.

### Pre-Treatment Measures

The seven outcome variables measured before treatment:

1. **Trust in IRS**: How much do you trust the IRS to act in the best interests of the American public?
2. **Opinion of the IRS**: Overall, how favorable is your opinion of the IRS? (1 = Very unfavorable, 7 = Very favorable)
3. **Opinion of IRS agents**: Overall, how favorable is your opinion of IRS agents? (1 = Very unfavorable, 7 = Very favorable)
4. **Perception of civil servants**: Most career civil servants are… (1 = Self-serving bureaucrats, 7 = Dedicated public servants)
5. **Opinion on IRS resources**: How much do you agree with the following statement? It is important for the IRS to have resources to enforce tax laws against wealthy tax evaders. (1 = Strongly disagree, 7 = Strongly agree)
6. **Approval of IRS workforce cuts**: The Trump administration, via the Department of Government Efficiency (DOGE), has reduced the IRS workforce by over 25%. How do you feel about this policy? (1 = Strongly disapprove, 7 = Strongly approve)
7. **Approval of Trump administration**: Do you approve of the Trump administration? (1 = Strongly disapprove, 7 = Strongly approve)

### Correlation Matrix

```{r}
#| label: correlation-matrix
#| fig-cap: "Correlation matrix of pre-treatment IRS/federal worker attitude measures"
#| fig-width: 7
#| fig-height: 6

library(corrplot)

# Select pre-treatment variables
pre_vars <- d |>
  select(
    `Trust in IRS` = trust_irs_pre_1,
    `Opinion of IRS` = fav_irs_pre_1,
    `Opinion of IRS agents` = agents_pre_1,
    `Civil servants` = servants_pre_1,
    `IRS enforcement` = enforce_pre_1,
    `DOGE approval` = doge_pre_1,
    `Trump approval` = trump_pre_1
  )

# Compute correlation matrix
cor_matrix <- cor(pre_vars, use = "pairwise.complete.obs")
...
cor_matrix |>
  ...
  gt::gt() |>
  gt::tab_header(title = "Correlation Matrix: Pre-Treatment Measures") |>
...
```{r}
#| label: pca-analysis
#| fig-cap: "PCA of pre-treatment IRS/federal worker attitude measures"

# Remove rows with missing values for PCA
pre_vars_complete <- na.omit(pre_vars)

# Run PCA with scaling
pca_result <- prcomp(pre_vars_complete, scale. = TRUE, center = TRUE)
...
scree_data |>
  mutate(
    ...
    `Variance Explained` = sprintf("%.1f%%", `Variance Explained`),
    `Cumulative Variance` = sprintf("%.1f%%", `Cumulative Variance`)
  ) |>
  gt::gt() |>
  gt::tab_header(title = "PCA Variance Explained")
```

```{r}
#| label: scree-plot
#| fig-cap: "Scree plot showing eigenvalues and cumulative variance explained"
#| fig-width: 8
#| fig-height: 4
...
p_scree <- ggplot(scree_data, aes(x = PC, y = Eigenvalue)) +
  ...
  theme_bw() +
  annotate("text", x = 6.5, y = 1.15, label = "Kaiser criterion (λ = 1)",
           color = "red", size = 3)

# Cumulative variance plot
p_cumvar <- ggplot(scree_data, aes(x = PC, y = `Cumulative Variance`)) +
...
```{r}
#| label: pca-loadings
#| fig-cap: "PCA component loadings"
#| fig-width: 8
#| fig-height: 5
...
# Display loadings table (first 3 components)
loadings_df |>
  select(Variable, PC1, PC2, PC3) |>
  mutate(across(starts_with("PC"), ~round(., 3))) |>
  gt::gt() |>
  gt::tab_header(title = "PCA Loadings (First 3 Components)") |>
  gt::tab_footnote(
    footnote = "Loadings show correlation between each variable and the principal component."
  )

# Visualize loadings as heatmap
loadings_long <- loadings_df |>
  pivot_longer(-Variable, names_to = "Component", values_to = "Loading") |>
  mutate(Component = factor(Component, levels = paste0("PC", 1:7)))
...
```{r}
#| label: pca-biplot
#| fig-cap: "PCA biplot showing observations and variable loadings"
#| fig-width: 7
#| fig-height: 6
...
loading_scale <- 3  # Scale factor for arrows
...
var_pc1 <- var_explained[1] * 100
var_pc2 <- var_explained[2] * 100
var_2comp <- cum_var[2] * 100

# Check if DOGE and Trump load together
doge_trump_pc1_same_sign <- sign(loadings_df$PC1[loadings_df$Variable == "DOGE approval"]) ==
                             sign(loadings_df$PC1[loadings_df$Variable == "Trump approval"])

# Average absolute correlation
avg_abs_cor <- mean(abs(cor_matrix[lower.tri(cor_matrix)]))

cat(sprintf("
**Key Findings:**
- **Kaiser criterion**: %d component(s) have eigenvalue ≥ 1
- **PC1** explains %.1f%% of variance; **PC1 + PC2** explain %.1f%%
- **Average absolute correlation** among measures: %.2f
- The measures %s tightly correlated overall
",
n_components_kaiser,
var_pc1,
var_2comp,
avg_abs_cor,
ifelse(avg_abs_cor > 0.5, "are moderately to", ifelse(avg_abs_cor > 0.3, "are weakly to moderately", "are not"))
))
```
